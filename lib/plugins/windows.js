      ...
      async.series([
        function setxPath(next) {
          return description.expand && description.expand.path
            ? spawnBatch('setx', [description.expand.path.slice(1)], next)
            : next();
        },
        async.apply(spawnBatch, '/program files/microsoft sdks/windows/v7.1/bin/setenv.cmd', [cpu, '/release']),
...
exports.init = function (callback) {
  ...
  function normalizeScripts(version, dir, next) {
    if (/0\.8/.test(version)) {
      return fs.rename(
        path.join(dir, version, 'nodejsvars.bat'),
        path.join(dir, version, 'nodevars.bat'),
        next
      );
    }
