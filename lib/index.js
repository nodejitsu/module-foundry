...
app.build = function (options, callback) {
  ...
  tmp.dir({ dir: app.config.get('directories:tmp') }, function (err, dir) {
    ...
    //
    if (options.registry) {
      registry = safeParse(options.registry);
      //
      ...
      if (registry.auth) {
        ...
        // We also must always-auth if we are using a private registry
        //
        jobDescription.env.npm_config_always_auth = "true";
        jobDescription.options = (jobDescription.options || []).concat(['--_auth=' + auth]);
      }

      jobDescription.env.npm_config_registry = registry.url;
    }
